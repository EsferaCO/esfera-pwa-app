...
export default function App() {
  ...
  const totalFunds = useMemo(() => {
    let sum = 0;
    accounts.forEach((a) => { sum += computedAccountBalances.get(a.id) || 0; });
    return sum;
  ...
  if (view === "expense") return <Expense goBack={nav.goHome} accounts={accounts} categories={categories} addTxn={addTxn} deleteTxn={deleteTxn} txns={txns} />;
  if (view === "debts") return <Debts goBack={nav.goHome} debts={debts} setDebts={setDebts} addTxn={addTxn} txns={txns} accounts={accounts} categories={categories} />;
